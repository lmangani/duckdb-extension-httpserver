...
void duckdb::ResultSerializer::SerializeChunk(const DataChunk &chunk,
                                              vector<string> &names,
...
yyjson_mut_val *duckdb::ResultSerializer::SerializeRowAsArray(
    const DataChunk &chunk, idx_t row_idx, vector<LogicalType> &types) {
...
yyjson_mut_val *duckdb::ResultSerializer::SerializeRowAsObject(
    const DataChunk &chunk, idx_t row_idx, vector<string> &names,
    vector<LogicalType> &types) {
  const auto column_count = chunk.ColumnCount();
...
void duckdb::ResultSerializer::SerializeValue( // NOLINT(*-no-recursion)
    yyjson_mut_val *parent, const Value &value, optional_ptr<string> name,
  ...
  case LogicalTypeId::DECIMAL: {
    ...
    if (std::isnan(real_val) || std::isinf(real_val)) {
      if (set_invalid_values_to_null) {
        ...
        const auto castedValue =
            value.DefaultCastAs(LogicalTypeId::VARCHAR).GetValue<string>();
  ...
  case LogicalTypeId::VARINT:
    val = yyjson_mut_strcpy(
        doc,
        value.DefaultCastAs(LogicalTypeId::VARCHAR).GetValue<string>().c_str());
    break;
    // UUID
  ...
  if (!val) {
    throw SerializationException("Could not serialize value of type " +
                                 type.ToString());
  }
  if (!name) {
