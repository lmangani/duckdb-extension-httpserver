	...
	if (debug_env != nullptr && std::string(debug_env) == "1") {
		global_state.server->set_logger(
		    [](const duckdb_httplib_openssl::Request &req, const duckdb_httplib_openssl::Response &res) {
			    ...
			    fprintf(stdout, "[%s] %s %s - %d - from %s:%d\r\n", timestr, req.method.c_str(), req.path.c_str(),
			            res.status, req.remote_addr.c_str(), req.remote_port);
			    fflush(stdout);
		    });
		...
		global_state.server->set_logger(
		    [](const duckdb_httplib_openssl::Request &req, const duckdb_httplib_openssl::Response &res) {
			    syslog(LOG_INFO, "%s %s - %d - from %s:%d", req.method.c_str(), req.path.c_str(), res.status,
